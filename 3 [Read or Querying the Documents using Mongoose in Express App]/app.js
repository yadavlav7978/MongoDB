...
mongoose.connect("mongodb://127.0.0.1:27017/Lav")
...
// This function is used to find student documents in the Students collection where the active field is set to true and only return the name field.
// The select() method takes an object as an argument, which is used to specify the fields that should be returned in the results. In this case, the select() method is used to specify that only the name field should be returned.

const findDocument = async () => {
  try {
    const result = await Students.find({ active: true }).select({ name: 1 });
    console.log(result);
  } catch (err) {
    console.log(err);
  }
